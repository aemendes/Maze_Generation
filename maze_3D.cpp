...
class Camera {
	...
	public:
		...
		void move(glm::vec3 offset){
			pos += offset*moveSensivity*vec3(1,1,1);
		}
		...
		void rotate(GLfloat xoffset, GLfloat yoffset)		{
			...
			glm::vec3 newFront;
			newFront.x = cos(glm::radians(pitch)) * cos(glm::radians(yaw))*2;
...
int main( void ){
    ...
    // Open a window
    window = glfwCreateWindow( WindowWidth, WindowHeight, "Maze 3D", NULL, NULL);
    ...
    printf("cube normals \n %f", maze.cube_normals[7]);

    // render scene for each frame
	Camera camera(&View, glm::vec3(maze.initial_cell ,6 , maze.initial_cell));
    do{
